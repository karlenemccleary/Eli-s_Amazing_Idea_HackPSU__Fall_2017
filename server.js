...
app.get("/historicals", function(req, res) {
  console.log("/historicals called with " + req.query.symbol)
  getHistoricalStocks(req.query.symbol, (hist)=> {res.send(hist)})
})

app.get("/twitterTone", function(req, res) {
  console.log("/twitterTone called with " + req.query.twitterHandle)
  var textInput;
  getTweets(req.query.twitterHandle, (twitterText)=> {
    getToneAnalysis(twitterText, (toneAnalysis)=> {
...
function getTweets(twitterHandle, callback) {
...
function getToneAnalysis(textInput, callback) {
  ...
  toneAnalyzer.tone({ text: textInput },
    function(err, tone) {
      if (err)
        console.log(err);
      else {
...
//Robinhood API
var credentials = {
    username: 'aidenchiavatti@gmail.com',
    password: 'aiden123red'
};

function getHistoricalStocks(stockSymbol, callback) {
  console.log("getting stocks")
  var historicals = [];
  var Robinhood = require('robinhood')(credentials, function(){
      //{interval=5minute|10minute (required) span=week|day| }
   
      Robinhood.historicals(stockSymbol, '10minute', 'day', function(err, response, body){
          if(err){
              console.error(err);
          }else{
              console.log("got historicals");
              for(i = 0; i < body["historicals"].length; i++) {
                  historicals.push(body["historicals"][i].open_price)
              }
              console.log(JSON.stringify(historicals));
              callback(historicals)
              //return historicals
              //callback()
              //             
